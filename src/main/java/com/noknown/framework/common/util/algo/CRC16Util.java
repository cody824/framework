...
public class CRC16Util {
	...
	private static final int[] TABLE8005 = { 0x0000, 0xC0C1, 0xC181, 0x0140,
	...
	private static final char TABLE1021[] = { /* CRC1021余式表 */
	...
	public static int getCRC8005ByTable(byte[] bytes) {
		int crc = 0x0000;

		for (byte b : bytes) {
			crc = (crc >>> 8) ^ TABLE8005[(crc ^ b) & 0xff];
		}
		return crc;
	}
	
	public static int getCRC8005(byte[] data) {
		int xda, xdapoly;
		int i, j, xdabit;
		xda = 0x0000 ; 
	...
	public static int getCRC1021(byte[] data) {
		int xda, xdapoly;
		int i, j, xdabit;
		xda = 0x0000 ; 
		xdapoly = 0x1021; // (X**16 + X**15 + X**2 + 1)
	...
	public static int getCRC1021ByTable(byte[] bytes) {
		int crc = 0x0000;

		for (byte b : bytes) {
			crc = (crc >>> 8) ^ TABLE1021[(crc ^ b) & 0xff];
		}
		return crc;
	}
	...
	public static int bytes2int(byte[] bytes) {
        int s = (int)(bytes[1] & 0xFF);
        s |= (bytes[0] << 8) & 0xFF00;
        return s;
    }
	
	public static byte[] getCRC8005Bytes(byte[] bytes) {
		return int2bytes(getCRC8005(bytes));
	}
	
	public static byte[] getCRC1021Bytes(byte[] bytes) {
		return int2bytes(getCRC1021(bytes));
	}

	public static byte[] getCRC8005BytesByTable(byte[] bytes) {
		return int2bytes(getCRC8005ByTable(bytes));
	}
	
	public static byte[] getCRC1021BytesByTable(byte[] bytes) {
		return int2bytes(getCRC1021ByTable(bytes));
	}
	
    public static boolean crc8005check(byte[] src, int crc){
		byte[] cbs = new byte[src.length + 2];
		System.arraycopy(src, 0, cbs, 0, src.length);
		cbs[cbs.length - 2] =  (byte) (crc & 0xff);
		cbs[cbs.length - 1] = (byte) (crc >> 8);
		int check = getCRC8005ByTable(cbs);
		return check == 0;
	}
    
    public static boolean crc8005check(byte[] src, byte[] crc){
		byte[] cbs = new byte[src.length + 2];
		System.arraycopy(src, 0, cbs, 0, src.length);
		cbs[cbs.length - 2] =  crc[1];
		cbs[cbs.length - 1] = crc[0];
		int check = getCRC8005ByTable(cbs);
		return check == 0;
	}
    
    public static boolean crc1021check(byte[] src, int crc){
  		byte[] cbs = new byte[src.length + 2];
  		System.arraycopy(src, 0, cbs, 0, src.length);
  		cbs[cbs.length - 2] =  (byte) (crc & 0xff);
  		cbs[cbs.length - 1] = (byte) (crc >> 8);
  		int check = getCRC1021ByTable(cbs);
  		return check == 0;
  	}
    
    public static boolean crc1021check(byte[] src, byte[] crc){
  		byte[] cbs = new byte[src.length + 2];
  		System.arraycopy(src, 0, cbs, 0, src.length);
  		cbs[cbs.length - 2] =  crc[1];
  		cbs[cbs.length - 1] =  crc[0];
...
//       int crc = getCRC8005(test);
       byte[] crc = CRC16Util.getCRC8005BytesByTable(test);
       System.out.println(Base16.encode(crc));
        System.out.println(crc8005check(test, crc));
