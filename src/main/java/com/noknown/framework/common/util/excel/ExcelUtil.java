...
public class ExcelUtil {
	 ...
	 * @throws IOException
	 */
	public static List<Map<String, Object>> getTemplateFile(String templateFileName) throws IOException {
		...
		for (int i = 0; i < numOfSheet; i++) {
			...
			templateMap.add(new HashMap<String, Object>());
			readSheet(templateMap.get(i), sheet);
		}
		fis.close();
		return templateMap;
	}
	 ...
	 * @param sheet
	 */
	private static void readSheet(Map<String, Object> keyMap, Sheet sheet) {
...
//              Cell cellIn = rowIn.getCell((short) k);
				...
				if (cellValue.length() > 2 && "${".equals(cellValue.substring(0, 2))) {
					...
					String key = cellValue.substring(3, cellValue.length() - 1);
	 ...
	 * @return
	 */
	public static CellStyle getStyle(Map<String, Object> keyMap, String key, Workbook wb) {
		...
		if (value == null) {
			...
			if (isCellDateFormatted(cellStyle)) {
				if (value instanceof Long) {
					cellIn.setCellValue(new Date((long) value));
	 ...
	 */
	public static int[] getPos(Map<String, Object> keyMap, String key) {
		int[] ret = new int[0];
